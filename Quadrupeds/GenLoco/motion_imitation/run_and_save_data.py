...
def test(model, env, num_procs, num_episodes=None):
  ...
  while episode_count < num_local_episodes:
    ...
    actions.append(a)
    print(f'ep: {episode_count}/{num_local_episodes}, step: {step}')
    step += 1
  ...
  return observations, episode_starts, actions
...
def main():
  ...
  if args.mode == "train":
      ...
      observations, episode_starts, actions = test(model=model,
           env=env,
           num_procs=num_procs,
           num_episodes=args.num_test_episodes)

      save_loc = f'../GenLocoData'
      save_info = f'{args.robot}'
      if 'pace_model' in args.model_file:
        save_info += '_pace'
      elif 'spin_model' in args.model_file:
        save_info += '_spin'
      
      os.makedirs(f'{save_loc}', exist_ok=True)
      np.save(f'{save_loc}/observations_{save_info}.npy', np.array(observations))
      np.save(f'{save_loc}/episode_starts_{save_info}.npy', np.array(episode_starts))
      np.save(f'{save_loc}/actions_{save_info}.npy', np.array(actions))

...
class BaseAviary(gym.Env):
    ...
    def __init__(self,
        ...
        self.DW_COEFF_1, \
        self.DW_COEFF_2, \
        self.DW_COEFF_3 = self._parseURDFParameters(length_scaling_factor, mass_scaling_factor)
    ...
    def _parseURDFParameters(self, length_scaling_factor=1.0, mass_scaling_factor = 1.0):
        ...
        M = mass_scaling_factor * float(URDF_TREE[1][0][1].attrib['value'])
        L = length_scaling_factor * float(URDF_TREE[0].attrib['arm'])
        THRUST2WEIGHT_RATIO = float(URDF_TREE[0].attrib['thrust2weight'])
        IXX = mass_scaling_factor * length_scaling_factor**2 * float(URDF_TREE[1][0][2].attrib['ixx'])
        IYY = mass_scaling_factor * length_scaling_factor**2 * float(URDF_TREE[1][0][2].attrib['iyy'])
        IZZ = mass_scaling_factor * length_scaling_factor**2 * float(URDF_TREE[1][0][2].attrib['izz'])
        J = np.diag([IXX, IYY, IZZ])
        J_INV = np.linalg.inv(J)
        KF = float(URDF_TREE[0].attrib['kf'])
        KM = float(URDF_TREE[0].attrib['km'])
        COLLISION_H = length_scaling_factor * float(URDF_TREE[1][2][1][0].attrib['length'])
        COLLISION_R = length_scaling_factor * float(URDF_TREE[1][2][1][0].attrib['radius'])
        COLLISION_SHAPE_OFFSETS = [float(s) for s in URDF_TREE[1][2][0].attrib['xyz'].split(' ')]
        COLLISION_Z_OFFSET = COLLISION_SHAPE_OFFSETS[2]
        MAX_SPEED_KMH = float(URDF_TREE[0].attrib['max_speed_kmh'])
        GND_EFF_COEFF = float(URDF_TREE[0].attrib['gnd_eff_coeff'])
        PROP_RADIUS = length_scaling_factor * float(URDF_TREE[0].attrib['prop_radius'])
        ...
        print(f'Drones parameters are {M=}, {L=}, {J=}, {J_INV=}, {KF=}, {KM=}')

...
class Logger(object):
    ...
    def __init__(self,
        ...
        # Modified OG self.states and self.controls above to following
        ...
        self.states = np.zeros((num_drones, 20, duration_sec*self.LOGGING_FREQ_HZ))
        self.controls = np.zeros((num_drones, 10, duration_sec*self.LOGGING_FREQ_HZ))
    ...
    def log(self,
        ...
        """
        if drone < 0 or drone >= self.NUM_DRONES or timestamp < 0 or len(state) != 20 or len(control) != 10:
        ...
        if current_counter >= self.timestamps.shape[1]:
            ...
            self.states = np.concatenate((self.states, np.zeros((self.NUM_DRONES, 20, 1))), axis=2) # OG: 16 instead of 20
            self.controls = np.concatenate((self.controls, np.zeros((self.NUM_DRONES, 10, 1))), axis=2) # OG: 12 instead of 10
        ...
        #### OG: Re-order the kinematic obs (of most Aviaries) #########
        # OG: States were saved as (x, y, z, vx, vy, vz, r, p, y, wr, wp, wy, rpm1, rpm2, rpm3, rpm4)
        # self.states[drone, :, current_counter] = np.hstack([state[0:3], state[10:13], state[7:10], state[13:20]])

        # now States are saved fully as (x, y, z, q0, q1, q2, q3, r, p, y, vx, vy, vz, wr, wp, wy, rpm1, rpm2, rpm3, rpm4) 
        self.states[drone, :, current_counter] = state[0:]
    ...
    def plot(self, pwm=False):
        ...
        # Current: States are (x, y, z, q0, q1, q2, q3, r, p, y, vx, vy, vz, wr, wp, wy, rpm1, rpm2, rpm3, rpm4) 
        # OG: state was saved as (x, y, z, vx, vy, vz, r, p, y, wr, wp, wy, rpm1, rpm2, rpm3, rpm4)
        ...
        row = 3
        for j in range(self.NUM_DRONES):
            rdot = np.hstack([0, (self.states[j, 7, 1:] - self.states[j, 7, 0:-1]) * self.LOGGING_FREQ_HZ ])
            axs[row, col].plot(t, rdot, label="drone_"+str(j))
        axs[row, col].set_xlabel('time')
        axs[row, col].set_ylabel('rdot (rad/s)')
        row = 4
        for j in range(self.NUM_DRONES):
            pdot = np.hstack([0, (self.states[j, 8, 1:] - self.states[j, 8, 0:-1]) * self.LOGGING_FREQ_HZ ])
            axs[row, col].plot(t, pdot, label="drone_"+str(j))
        axs[row, col].set_xlabel('time')
        axs[row, col].set_ylabel('pdot (rad/s)')
        row = 5
        for j in range(self.NUM_DRONES):
            ydot = np.hstack([0, (self.states[j, 9, 1:] - self.states[j, 9, 0:-1]) * self.LOGGING_FREQ_HZ ])
